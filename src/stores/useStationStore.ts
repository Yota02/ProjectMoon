...
export const useStationStore = defineStore(
  ...
  () => {
    ...
    gameEvents.on('day-elapsed', ({ daysPassed, currentDate }) => {
      ...
      const { argentPerDay, sciencePerDay, carburantPerDay, materiauxRaresPerDay } =
        stationBonuses.value
      ...
      if (materiauxRaresPerDay !== 0)
        resourceStore.addMateriauxRares(materiauxRaresPerDay * daysPassed)
    ...
    const stationBonuses = computed(() => {
      ...
      const energyStats: Record<
        string,
        { prod: number; cons: number; storage: number; shielding: number }
      > = {}
      ...
      stations.value.forEach((station) => {
        ...
        energyStats[station.id] = {
          prod: sProd,
          cons: sCons,
          storage: sStorage,
          shielding: sShielding,
        }
    ...
    const createStation = (name: string, orbitBodyId: string) => {
      ...
      const cost = { argent: 2000 }

      if (resourceStore.argent < cost.argent) {
    ...
    const addModuleToStation = (
      ...
      if (resourceStore.argent < module.cost.argent) {
        return { success: false, message: 'Ressources insuffisantes' }
      }
      ...
      stations.value.forEach((station) => {
        if (gameStore.elapsedDays >= station.constructionFinishedDay && station.resources) {
          ...
          if ((crewCount > 0 || civilianCount > 0) && station.resources) {
            ...
            if (!isWaterRecycled) {
              station.resources.eau = Math.max(
                0,
                station.resources.eau - totalConsumptionFactor * daysPassed,
              )
            }

            station.resources.o2 = Math.max(
              0,
              station.resources.o2 - totalConsumptionFactor * o2Factor * daysPassed,
            )
          }
          ...
          if (isBlackout) {
            ...
            if (Math.random() < 0.1 * daysPassed) {
              trainingStore.log(
                `[ALERTE] Blackout total sur ${station.name} ! Le moral s'effondre.`,
              )
            }
          }
          ...
          if (station.moral < 20 && !station.isOnStrike) {
            ...
            gameEvents.emit('station-strike-started', {
              stationId: station.id,
              stationName: station.name,
            })
            trainingStore.log(
              `[SOCIAL] Grève générale sur ${station.name} ! La production de Science est arrêtée.`,
            )
          } else if (station.moral > 40 && station.isOnStrike) {
            station.isOnStrike = false
            trainingStore.log(
              `[SOCIAL] Fin de la grève sur ${station.name}. Les chercheurs reprennent le travail.`,
            )
          }
          ...
          if (station.moral < 10 && station.civilianPopulation > 0) {
            ...
            if (departures > 0) {
              trainingStore.log(
                `[EXODE] ${departures} civils ont quitté ${station.name} pour la concurrence (moral trop bas).`,
              )
            }
          }
      ...
      if (station.astronautIds.length > 0) {
        ...
        if (Math.random() < 0.33 * daysPassed) {
          ...
          if (victimId !== undefined) {
            ...
            trainingStore.log(
              `[CATASTROPHE] Un membre d'équipage de ${station.name} a succombé par manque d'O2.`,
            )
          }
        }
      }
        ...
        if (station.constructionFinishedDay <= gameStore.elapsedDays) {
          if (station.resources) {
            ...
            if (station.civilianPopulation < maxCivilian && station.moral > 40) {
              ...
              station.civilianPopulation = Math.min(
                maxCivilian,
                station.civilianPopulation + growth,
              )
            }
          }
        }
