  ...
  () => {
    ...
    gameEvents.on('day-elapsed', ({ daysPassed }) => {
      ...
      const { totalArgentPerDay, totalSciencePerDay, totalCarburantPerDay } =
        allBasesAdjacencyBonuses.value
    ...
    // Debug initialization removed to ensure a clean start
    ...
    const isTileInOwnedParcel = (x: number, y: number): boolean => {
      ...
      return parcels.value.some(
        (parcel) =>
          currentBase.value?.ownedParcels.has(parcel.id) &&
          parcel.parcelX === parcelX &&
          parcel.parcelY === parcelY,
      )
    }
      ...
      for (let i = 0; i < buildings.length; i++) {
        for (let j = i + 1; j < buildings.length; j++) {
          ...
          const matchesForward =
            first.buildingId === rule.sourceBuildingId &&
            second.buildingId === rule.targetBuildingId
          const matchesBackward =
            first.buildingId === rule.targetBuildingId &&
            second.buildingId === rule.sourceBuildingId
    ...
    const adjacencyBonuses = computed(() => {
      if (!currentBase.value)
        return {
          activeBonuses: [],
          totalArgentPerDay: 0,
          totalSciencePerDay: 0,
          totalCarburantPerDay: 0,
        }
      ...
      for (const rule of BUILDING_ADJACENCY_RULES) {
        ...
        const carburantPerDay =
          rule.carburantPerDay * triggerCount * (0.5 + (zoneMult.energy + zoneMult.water) / 2)
