<template>
  <div
    class="relative w-full h-full bg-slate-950 overflow-hidden cursor-move"
    @wheel="handleWheel"
    @mousedown="startPan"
    @mousemove="doPan"
    @mouseup="stopPan"
  ...
  >
    ...
    <div class="absolute inset-0 pointer-events-none opacity-50">
      <div
        ...
        :style="{
          transform: `translate3d(${-offsetX * layer.depth * 0.6}px, ${-offsetY * layer.depth * 0.6}px, 0)`,
        }"
      >
        <div
          v-for="star in layer.stars"
          :key="`bg-star-${layer.id}-${star.id}`"
    ...
    <div class="absolute top-4 left-4 z-20 flex flex-col gap-2">
      <div
        class="bg-slate-900/60 border border-slate-700/50 p-4 rounded-2xl backdrop-blur-xl shadow-2xl"
      >
        ...
        <div class="mt-4 flex items-center gap-4">
          <button
            @click="resetView"
            class="px-3 py-1 bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/30 text-blue-400 text-[10px] font-black uppercase rounded-lg transition-all"
          >
            Réinitialiser
          </button>
          <span class="text-[10px] font-black text-slate-500 uppercase"
            >Zoom: {{ (scale * 100).toFixed(0) }}%</span
          >
        </div>
      </div>
      ...
      >
        <div class="space-y-2">
          ...
          >
            <div
              class="w-4 h-4 rounded-full overflow-hidden border border-white/20 group-hover:scale-125 transition-transform"
            >
              <img
            ...
            <span
              class="text-[10px] font-bold text-slate-400 group-hover:text-white uppercase tracking-wider transition-colors"
              >{{ planet.name }}</span
            >
          </div>
        </div>
    ...
    <svg class="w-full h-full relative z-10" :viewBox="viewBox">
      ...
      <g v-for="travel in travelPositions" :key="'path-' + travel.id" class="pointer-events-none">
        <line
          :x1="getBodyPositionAt(travel.originId, travel.departureDay).x"
      ...
      <!-- Celestial Bodies -->
      <g
        v-for="planet in driftedPlanetPositions"
        :key="planet.id"
      ...
      >
        ...
        <circle
          ...
          :fill="
            planet.id === 'sun'
              ? 'url(#sunGlow)'
              : 'radial-gradient(circle, ' + planet.color + '44 0%, transparent 70%)'
          "
        ...
        <!-- Planet Image -->
        <image
          :href="assetPath(`assets/images/planets/${planet.id}.png`)"
          :x="-planet.radius"
          :y="-planet.radius"
        ...
        <g
          v-if="planet.hazards && planet.hazards.length > 0"
          :transform="`translate(${planet.radius + 5}, -${planet.radius + 5})`"
        >
      ...
      <!-- Active Travels -->
      <g
        v-for="travel in travelPositions"
        :key="travel.id"
        :transform="`translate(${travel.x}, ${travel.y})`"
        class="pointer-events-none"
      >
        <image
          :href="assetPath('assets/images/ui/satellite.png')"
          :x="-4 / scale"
          :y="-4 / scale"
          :width="8 / scale"
          :height="8 / scale"
      ...
      >
        ...
        <!-- Station icon -->
        <image
          v-else-if="obj.type === 'station'"
          :href="assetPath('assets/images/ui/station.png')"
          :x="-2.5 / scale"
          :y="-2.5 / scale"
          :width="5 / scale"
          :height="5 / scale"
    ...
    >
      <div v-if="selectedPlanet" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div
            class="bg-slate-950/50 p-4 rounded-xl border border-slate-800 flex items-center gap-4"
          >
            <div
        ...
        <div v-if="selectedPlanet.zones && selectedPlanet.zones.length > 0" class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-3">
            <div
              v-for="zone in selectedPlanet.zones"
              :key="zone.id"
              ...
              <div class="flex justify-between items-start">
                <div class="flex-grow">
                  <h5 class="text-white font-bold flex items-center gap-2">
                    ...
                    <span
                      v-if="!zone.unlocked"
                      class="text-[9px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded uppercase font-black"
                      >Verrouillé</span
                    >
                    <span
                      v-if="explorationStore.getProspectionResult(zone.id)"
                      class="text-[9px] bg-emerald-900/40 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded uppercase font-black"
                      >Scanné & Prospecté</span
                    >
                  </h5>
                  ...
                  <!-- Scan Progress Bar -->
                  <div
                    v-if="zone.unlocked && !explorationStore.getProspectionResult(zone.id)"
                    class="mt-3 w-full max-w-xs"
                  >
                    <div class="flex justify-between items-center mb-1">
                      <span class="text-[8px] text-slate-500 uppercase font-black"
                        >Scan de surface</span
                      >
                      <span class="text-[8px] text-blue-400 font-black"
                        >{{ (zone.scanProgress || 0).toFixed(0) }}%</span
                      >
                    </div>
                    <div class="h-1 bg-slate-800 rounded-full overflow-hidden">
                      <div
                        class="h-full bg-blue-500 transition-all duration-300"
                        :style="{ width: (zone.scanProgress || 0) + '%' }"
                      ></div>
                    </div>
                  </div>
                </div>
                ...
                <div v-if="zone.unlocked" class="flex flex-col gap-2 ml-4">
                  ...
                  <router-link
                    v-if="zone.baseId"
                    :to="{ name: 'base', query: { zoneId: zone.id } }"
                  ...
                  <!-- Scan not finished -->
                  <div
                    v-else-if="(zone.scanProgress || 0) < 100"
                    class="flex flex-col gap-1 items-end"
                  >
                    <button
                      v-if="!isRoverScanning(zone.id)"
                    ...
                    >
                      {{
                        explorationStore.rovers.filter((r) => r.status === 'Ready').length > 0
                          ? 'Lancer Scan rover'
                          : 'Acheter Rover (50M)'
                      }}
                    </button>
                    <div
                      v-else
                      class="px-3 py-1.5 bg-slate-800 text-slate-500 text-[10px] font-black uppercase rounded-lg text-center flex items-center gap-2"
                    >
                  ...
                  <button
                    v-else-if="!explorationStore.getProspectionResult(zone.id)"
              ...
              >
                <div class="bg-blue-900/10 p-2 rounded-lg border border-blue-800/20 text-center">
                  ...
                  <p class="text-xs font-bold text-white">
                    {{
                      (
                        zone.resources.minerals *
                        (explorationStore.getProspectionResult(zone.id)?.mineralQuality || 1) *
                        100
                      ).toFixed(0)
                    }}%
                  </p>
                ...
                <div class="bg-blue-900/10 p-2 rounded-lg border border-blue-800/20 text-center">
                  ...
                  <p class="text-xs font-bold text-white">
                    {{
                      (
                        zone.resources.water *
                        (explorationStore.getProspectionResult(zone.id)?.waterQuality || 1) *
                        100
                      ).toFixed(0)
                    }}%
                  </p>
                ...
                <div class="bg-blue-900/10 p-2 rounded-lg border border-blue-800/20 text-center">
                  ...
                  <p class="text-xs font-bold text-white">
                    {{
                      (
                        zone.resources.energy *
                        (explorationStore.getProspectionResult(zone.id)?.energyQuality || 1) *
                        100
                      ).toFixed(0)
                    }}%
                  </p>
                ...
                <div class="bg-blue-900/10 p-2 rounded-lg border border-blue-800/20 text-center">
                  ...
                  <p class="text-xs font-bold text-white">
                    {{
                      (
                        zone.resources.science *
                        (explorationStore.getProspectionResult(zone.id)?.scienceQuality || 1) *
                        100
                      ).toFixed(0)
                    }}%
                  </p>
        ...
        <div class="space-y-4">
          ...
          <div
            v-if="orbitalStations.length === 0"
            class="py-12 text-center bg-slate-950/30 rounded-2xl border border-dashed border-slate-800"
          >
            ...
            <router-link
              ...
              to="/stations"
              class="mt-4 inline-block text-blue-400 text-xs font-bold hover:underline"
            >
          ...
          <div v-else class="space-y-3">
            <div
              v-for="station in orbitalStations"
              :key="station.id"
            ...
            >
              <div>
                ...
                <p class="text-[10px] text-slate-500 mt-1">
                  {{ station.moduleIds.length }} modules •
                  {{ station.astronautIds.length }} astronautes
                </p>
              </div>
              <div class="flex items-center gap-4">
                ...
                >
                  ...
                  <p class="text-[10px] text-slate-500">
                    {{ station.constructionFinishedDay - gameStore.elapsedDays }}j restants
                  </p>
                </div>
                <div v-else class="text-right">
                ...
                <router-link
                  to="/stations"
                  class="p-2 bg-slate-800 rounded-lg text-slate-400 group-hover:text-white transition-colors"
                >
    ...
    <div
      v-if="showProspectionModal"
      class="absolute inset-0 z-[100] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4"
    >
      <div class="w-full max-w-xl">
        ...
        />
        <button
          @click="showProspectionModal = false"
          class="mt-4 w-full text-slate-500 font-bold uppercase text-[10px] hover:text-white transition-colors"
        >
          Annuler l'opération
        </button>
      </div>
...
const orbitalSatellites = computed(() => {
  if (!selectedPlanetId.value) return []
  return satelliteStore.activeSatellites.filter(
    (s) => s.bodyId === selectedPlanetId.value && s.status === 'En Orbite',
  )
})
...
// State for Stars and Drift
const starLayers = ref<StarLayer[]>([])
const starfieldTime = ref(0)
const driftTime = ref(0)
let animationFrameId: number | null = null

const buildStarLayers = () => {
  const layerDefs = [
    { id: 'far', depth: 0.15, count: 120, maxRadius: 1.3, baseOpacity: 0.25 },
    { id: 'mid', depth: 0.35, count: 90, maxRadius: 1.8, baseOpacity: 0.32 },
    { id: 'near', depth: 0.55, count: 60, maxRadius: 2.2, baseOpacity: 0.4 },
  ]

  starLayers.value = layerDefs.map((layer) => ({
    id: layer.id,
    depth: layer.depth,
    stars: Array.from({ length: layer.count }, (_, index) => ({
      ...
      r: Math.max(0.6, Math.random() * layer.maxRadius),
      opacity: layer.baseOpacity + Math.random() * 0.45,
      twinkleSeed: Math.random() * Math.PI * 2,
    })),
  }))
}

const starStyle = (star: StarPoint) => {
  const twinkle = 0.65 + Math.sin(starfieldTime.value * 0.0015 + star.twinkleSeed) * 0.25
  return {
    left: `${star.x}px`,
    top: `${star.y}px`,
    width: `${star.r}px`,
    height: `${star.r}px`,
    opacity: Math.max(0.1, Math.min(1, star.opacity * twinkle)),
  }
}

const driftedPlanetPositions = computed(() => {
  return planetPositions.value.map((planet, index) => {
    if (planet.id === 'sun') return planet
    const driftX = Math.sin(driftTime.value * 0.00025 + index * 0.9) * 1.8
    const driftY = Math.cos(driftTime.value * 0.00022 + index * 1.1) * 1.2
    return {
      ...planet,
      x: planet.x + driftX,
      y: planet.y + driftY,
...
const animateScene = (timestamp: number) => {
  starfieldTime.value = timestamp
  driftTime.value = timestamp
...
const startScanning = (zoneId: string) => {
  ...
  if (readyRovers.length === 0) {
    ...
    const rover = readyRovers[0]
    if (!rover) return
    explorationStore.startScan(zoneId, rover.id)
  }
}
...
onMounted(() => {
  buildStarLayers()
  animationFrameId = window.requestAnimationFrame(animateScene)
  window.addEventListener('resize', buildStarLayers)
})
...
onUnmounted(() => {
  ...
  window.removeEventListener('resize', buildStarLayers)
})
</script>
