      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        >
          ...
          >
            <div class="sm:col-span-1">
              <label
                class="block text-[10px] uppercase font-black text-slate-500 mb-1.5 tracking-widest"
                >Base de Châssis</label
              >
              <select
                v-model="newDesignBase"
                class="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-300 focus:border-blue-500/50 outline-none"
              >
                <option v-for="d in fleetStore.availableDesigns" :key="d.id" :value="d.id">
                  {{ d.name }}
                </option>
              </select>
            </div>
            <div class="sm:col-span-1">
              ...
              <input
                ...
                type="text"
                placeholder="ex: Ariane 6, Falcon..."
                class="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-300 focus:border-blue-500/50 outline-none"
              />
            </div>
          ...
          <div
            v-else
            class="text-center py-4 bg-slate-950/50 rounded-xl border border-dashed border-slate-800"
          >
            <p class="text-xs text-slate-500 italic">
              Débloquez des châssis de base via la R&D pour concevoir vos propres modèles.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <p class="text-slate-500 text-sm mt-2 max-w-xs mx-auto">
            Recherchez de nouvelles technologies de lanceurs dans le centre de R&D pour commencer la
            production.
          </p>
          <router-link
            to="/rd"
            class="mt-6 inline-block text-blue-400 hover:text-blue-300 text-sm font-bold"
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div class="flex justify-between items-start mb-4">
              <div>
                <h4 class="font-bold text-slate-100 group-hover:text-blue-400 transition-colors">
                  {{ design.name }}
                </h4>
              ...
              <div class="bg-slate-800 p-2 rounded-lg">
                <BaseIcon
                  :name="design.type === 'launcher' ? 'rocket' : 'globe'"
            ...
            <div class="space-y-2 mb-6">
              <div class="flex justify-between text-[10px] uppercase font-bold tracking-wider">
                ...
                <span class="font-mono text-emerald-400 font-bold"
                  >{{ design.cost.toLocaleString() }} €</span
                >
              </div>
              <div class="flex justify-between text-[10px] uppercase font-bold tracking-wider">
              ...
              <div class="flex justify-between text-[10px] uppercase font-bold tracking-wider">
                <span class="text-slate-500">Charge Utile:</span>
                <span class="font-mono text-blue-300">{{ design.cargoCapacity }}</span>
              </div>
              <div class="flex justify-between text-[10px] uppercase font-bold tracking-wider">
                <span class="text-slate-500">Réutilisable:</span>
                <span
                  class="font-mono"
                  :class="design.isReusable ? 'text-emerald-400' : 'text-slate-500'"
                  >{{ design.isReusable ? 'OUI' : 'NON' }}</span
                >
              </div>
              <div class="flex justify-between text-[10px] uppercase font-bold tracking-wider">
                ...
                <span
                  class="font-mono"
                  :class="design.canReachMoon ? 'text-emerald-400' : 'text-slate-500'"
                  >{{ design.canReachMoon ? 'OUI' : 'NON' }}</span
                >
              </div>
              <div class="flex flex-col gap-1 pt-1">
                <span class="text-[9px] uppercase font-black text-slate-600 tracking-tighter"
                  >Orbites Supportées</span
                >
                <div class="flex flex-wrap gap-1">
                  <span
                    v-for="orbit in design.supportedOrbits"
                    :key="orbit"
                    class="px-1.5 py-0.5 bg-slate-800 text-[9px] font-mono text-slate-400 rounded border border-slate-700"
                  >
                    {{ orbit }}
                  </span>
                </div>
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <FleetItem
            ...
            :icon="
              fleetStore.designs.find((d) => d.id === item.designId)?.type === 'launcher'
                ? 'rocket'
                : 'globe'
            "
          />
...
const buildingItems = computed(() => fleetStore.items.filter((i) => i.status === 'En construction'))
</script>
